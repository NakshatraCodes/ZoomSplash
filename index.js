...
app.post("/unsplash", (req, res) => {
  ...
  function getPhoto(chatbotToken) {
    request(
      `https://api.unsplash.com/search/photos?query=${req.body.payload.cmd}&orientation=landscape&client_id=${process.env.unsplash_access_key}`,
      (error, body) => {
        if (error) {
          ...
          if (body.results.length === 0) {
            var errors = [
              {
                type: "section",
                sidebar_color: "#D72638",
                sections: [{ type: "message", text: "No Photos Found" }],
              },
            ];
            sendChat(errors, chatbotToken);
          } else {
            var photo = [
              {
                type: "section",
                sidebar_color: body.results[0].color,
                sections: [
                  {
                    type: "attachments",
                    img_url: body.results[0].urls.regular,
                    resource_url: body.results[0].user.links.html,
                    information: {
                      title: {
                        text: "Photo by " + body.results[0].user.name,
                      },
                      description: {
    ...
    request(
      ...
      (error, httpResponse, body) => {
        if (error) {
          ...
          console.log(body);
          pg.query(
            `UPDATE chatbot_token SET token = '${
              body.access_token
            }', expires_on = ${new Date().getTime() / 1000 + body.expires_in}`,
            (error, results) => {
              if (error) {
...
app.listen(port, () =>
  console.log(`Unsplash Chatbot for Zoom listening on port ${port}!`)
);
